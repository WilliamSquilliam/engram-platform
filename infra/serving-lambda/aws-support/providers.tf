# AWS provider + account guard. Mirrors serving-aws/providers.tf: this stack
# deploys ONLY to the Engram Dynamics account (808379776072) via the
...
provider "aws" {
  ...
  default_tags {
    tags = {
      ...
      Component = "gpu-serving-lambda-aws-support"
      ManagedBy = "terraform"
    }
  }
}
